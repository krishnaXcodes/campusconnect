...
import { FiHome, FiSearch, FiCompass, FiMessageSquare, FiHeart, FiPlusSquare, FiUser } from 'react-icons/fi';
...
export default function FloatingDock() {
  ...
  const navItems = [
    ...
    { name: 'Create', path: '/create-post', icon: FiPlusSquare, primary: true },
    { name: 'Messages', path: '/messages', icon: FiMessageSquare },
    { name: 'Campus', path: '/students', icon: MdSchool },
    { name: 'Profile', path: `/profile/${user?.username}`, icon: FiUser },
  ];
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `relative group flex items-center justify-center p-3 rounded-full transition-all duration-300 ${
                  item.primary 
                    ? 'bg-gradient-to-r from-aurora-blue to-aurora-purple text-white shadow-[0_0_15px_rgba(0,210,255,0.4)] hover:scale-110' 
                    : isActive 
                      ? 'bg-white/10 text-aurora-cyan' 
                      : 'text-dark-300 hover:text-white hover:bg-white/5 hover:-translate-y-1'
                }`
              }
            >
              <Icon className={item.primary ? 'text-2xl' : 'text-xl'} />
